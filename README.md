...
>
> 钱包、双向兑换与商店所有权已经完成前后端对接，统一比例为 `1 元宝 = 1000 铜钱`。知识挑战后端已经完成，完整答题前端、文物修复记录和 3D 模型解锁仍待开发。
...
本分支提供认证、数据库钱包、`1 元宝 = 1000 铜钱` 双向兑换、工具/场所/工作台购买与所有权，以及知识问答后端能力。商店当前商品价格均为 0，文物商品尚未提供；知识挑战仍缺少完整答题前端，修复关卡、文物修复记录和 3D 模型解锁也属于后续范围。
